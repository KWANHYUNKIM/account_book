...
export default function DashboardView({ onSessionClick }: DashboardViewProps) {
  ...
  // 거래가 있는 날짜 추출 (마지막 접근 날짜 기준)
  const markedDates = sortedSessions
    .map(session => new Date(session.lastAccessedAt))
    ...
    <div style={{ padding: '40px', marginLeft: '240px' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        <Calendar 
          ...
          onDateClick={(date) => {
            // 날짜 클릭 시 해당 날짜의 기록 필터링 (추후 구현 가능)
      ...
      <div style={{ marginBottom: '40px' }}>
        {sortedSessions.length > 0 ? (
          ...
          <div style={{
            ...
            <div style={{ fontSize: '18px', marginBottom: '8px' }}>최근 기록이 없습니다</div>
            <div style={{ fontSize: '14px' }}>기록 메뉴에서 새 기록을 만들어보세요</div>
          </div>
        )}
      </div>
